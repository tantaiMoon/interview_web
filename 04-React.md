...
Fiber 是 React 重新设计的协调架构，本质上是把原来不可中断的更新过程拆成更细粒度、可中断、可恢复的任务单元。
...
1. **16**：底层换成 Fiber（链表结构 + 可中断），但还是同步消费。
2. **16.3**：为异步渲染清理生命周期。
3. **16.6 / 16.8**：上层 API 升级（memo、lazy、Hooks），开发体验跟上。
4. **17**：基础设施改造（事件、JSX、effect 时机），为并发铺路。
5. **18**：并发渲染正式可用，自动批处理、Transitions、新 SSR。
6. **19**：把"客户端 + 服务端"统一起来（RSC、Actions），开发心智回归"写组件就是写 UI"。
