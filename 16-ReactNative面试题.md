...
1. **能够说明 React Native 不是简单的 WebView 包装**，而是通过 Bridge 调用原生组件渲染真正的原生 UI，这是与 Cordova/Ionic 等方案的本质区别。

2. **了解新架构（JSI、Fabric、TurboModules）的改进方向**：
   - JSI 解决了 Bridge 的性能瓶颈
   - Fabric 支持并发渲染和优先级调度
   - TurboModules 实现了按需加载，提升启动速度

3. **能够分析 React Native 的适用场景**，不盲目推崇或贬低，而是根据项目需求做出合理选择。

4. **了解热更新的原理和限制**，知道什么能更新、什么不能更新，以及各平台的政策限制。

5. **能够说出实际的性能优化经验**，如列表优化、动画优化、包体积优化等。
...
1. **能够清楚地解释 Bridge 的工作机制和性能瓶颈**，以及新架构如何解决这些问题。

2. **了解 JSI、Fabric、TurboModules 的作用和优势**，说明 React Native 正在向更高性能的方向演进。

3. **能够分析新架构的迁移成本和时机**，不盲目追新，而是根据项目情况做出合理决策。

4. **了解 Hermes 引擎的优势**，知道为什么 Android 默认使用 Hermes。

5. **能够说出实际的性能数据**，如启动速度提升 50%，通信延迟降低 10 倍等。
...
1. **能够解释虚拟化的原理**：只渲染可见部分，复用组件，减少内存占用。

2. **知道 FlatList 的性能优化参数**：
   - `initialNumToRender`：首次渲染数量
   - `maxToRenderPerBatch`：每批渲染数量
   - `windowSize`：维护的窗口大小
   - `removeClippedSubviews`：移除不可见视图
   - `getItemLayout`：提前告知 item 高度，优化滚动

3. **了解常见的性能陷阱**：
   - 不要在 ScrollView 中渲染大量数据
   - 不要在 FlatList 外层套 ScrollView
   - 不要嵌套 FlatList
...
4. **能够根据场景选择合适的组件**：
   ...
   - 大量列表数据 → FlatList
   - 分组列表 → SectionList
   - 混合内容 → FlatList + ListHeaderComponent
...
1. **能从"渲染/通信/资源"三个维度系统讲优化**，而不是只会说"加 memo"。
2. **能说出 `useNativeDriver` 的限制**——只支持非布局属性，体现对原生动画系统的理解。
3. **了解 Reanimated 的优势**：worklet 机制把更多动画逻辑下沉到原生层，能做手势驱动的复杂动画。
4. **知道用 React DevTools Profiler 和 Flipper Performance 定位瓶颈**，而不是凭感觉优化。
5. **了解 Hermes 引擎的字节码预编译机制**，能解释为什么它启动快。
...
1. **能讲清楚 Bridge 的异步本质和 JSON 序列化开销**，并说明为什么 TurboModule 能消除这些开销。
2. **了解三种通信模式（Callback/Promise/Event）的适用场景**，不局限于 Promise。
3. **知道线程切换的必要性**——这是新人最容易踩的坑（在子线程改 UI 导致崩溃）。
4. **能解释 Codegen 的价值**：从手写两边代码到从 Spec 单源生成，类型一致性自动保证。
5. **了解原生 UI 组件（Native Components）和原生模块的区别**：模块是方法调用，组件是渲染节点，两者注册和使用方式不同。
...
1. **理解热更新的边界**：能清晰说出"什么能更"和"什么不能更"。
2. **了解灰度发布机制**：CodePush 支持按 `rollout` 百分比逐步发布，控制风险。
3. **能讲苹果审核的合规点**：不能用热更新规避审核（如发版后改成游戏内付费功能），否则会被下架。
4. **了解替代方案**：自研热更新（pushy、阿里云 OTA）、使用 EAS Update（Expo 提供）等，各有取舍。
5. **知道版本兼容**：热更包必须和原生壳的 RN 版本匹配，跨大版本（如 0.68 → 0.72）必须重新发版。
...
1. **理解 i18n 不只是翻译**：复数、日期、货币、RTL 都是大坑。
2. **能讲清楚动态切换语言的实现**：监听 `i18n.changeLanguage`，配合 React 重渲染。
3. **了解服务端翻译方案**：大型应用文案通常存在 CMS，运行时动态加载,而不是打包进 bundle。
4. **知道翻译协作流程**：用 Crowdin、Lokalise 这类平台让翻译人员在线协作，导出 JSON 再集成到代码。
5. **了解 ICU MessageFormat**：i18next 默认语法的扩展，能表达更复杂的语言变化（如选择不同语句）。
...
1. **能讲清虚拟化的原理**：只渲染可见区域 + 缓冲区，复用 cell。
2. **知道 `getItemLayout` 的限制和价值**：必须定高，但能跳过测量阶段。
3. **能区分 React 重渲染和原生重排**：memo 优化的是 React reconcile，不是原生 layout。
4. **了解 FlashList 的实现思路**：cell pooling + recycling，是真正的"原生级"列表。
5. **从产品角度反问**：真的需要一次性 10000 条吗？大多数情况这是设计问题不是技术问题。
...
1. **能区分 PanResponder 和 react-native-gesture-handler**：前者跑在 JS 线程容易卡，后者跑在原生层平滑。
2. **理解 worklet 机制**：Reanimated 的核心，把 JS 函数编译成在 UI 线程独立运行的代码块。
3. **能讲手势冲突的解决思路**：长按、激活区、动态禁用其他手势，是真实业务里的高频问题。
4. **了解性能边界**：item 数量大时(1000+),拖拽时所有 item 都重算 transform 会卡——需要只动 hover 区域附近的 item。
5. **知道无障碍访问**：拖拽排序对屏幕阅读器用户不友好,要提供"上移/下移"按钮作为替代方案。
...
1. **`inverted` 和 `maintainVisibleContentPosition` 是聊天列表的两个杀手锏**，懂的人不多。
2. **图片消息预先占位**：消息体里带 `width/height`，占位 `View` 撑高，图片加载完不引起布局抖动。
3. **未读消息分隔线**：在数组里插入虚拟的"未读起点"项，进入聊天时滚到这个位置。
4. **WebSocket 状态管理**：网络断开重连后要补拉缺失消息（带 lastMessageId），防止丢消息。
5. **大型聊天群优化**：单聊天 10000+ 消息时即使虚拟化也吃力，可以考虑用 FlashList 或者按时间段分页持久化。
